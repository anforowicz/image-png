...
impl<R: BufRead> Decoder<R> {
    ...
    /// limits.bytes = 3;
    /// let file_reader = BufReader::new(File::open("tests/pngsuite/basi0g01.png").unwrap());
    /// let mut decoder = Decoder::new_with_limits(file_reader, limits);
...
pub struct Reader<R: BufRead> {
    ...
    /// Most recently read, already unfiltered line
    prev_row: Vec<u8>,
    /// Buffer for reading the next raw line
    curr_row: Vec<u8>,
    /// Output transformations
    transform: Transformations,
...
impl<R: BufRead> Reader<R> {
    ...
    fn read_until_image_data(&mut self) -> Result<(), DecodingError> {
        ...
        self.prev_row.clear();
        self.prev_row.resize(self.subframe.rowlen, 0);
    ...
    fn next_interlaced_row_impl(
        ...
        let row = &self.prev_row[1..rowlen];
    ...
    fn next_pass(&mut self) -> Option<(usize, InterlaceInfo)> {
        match self.subframe.interlace {
            InterlaceIter::Adam7(ref mut adam7) => {
                ...
                if last_pass != pass {
                    self.prev_row.clear();
                    self.prev_row.resize(rowlen, 0u8);
                }
    ...
    fn next_raw_interlaced_row(&mut self, rowlen: usize) -> Result<(), DecodingError> {
        if self.curr_row.len() != rowlen {
            self.curr_row.resize(rowlen, 0);
        }
        ...
        // Unfilter the row.
        let row = self.curr_row.as_mut_slice();
